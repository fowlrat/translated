...
Comdat IRは、COFFとELFオブジェクトファイルにおけるCOMDAT機能へのアクセ
スを提供します。

Comdatは、COMDATであることを示す特有のキーを名前として持ちます。この名
前を持つすべての大域オブジェクトのうち、リンカによって選ばれたオブジェ
クトのみが、最終的なオブジェクトに含まれます。エイリアスは、そのエイリ
アスが計算されたのと同じCOMDATがあればそのCOMDATに含まれます(?)。

Comdatは、リンカが複数のオブジェクトからどのようにオブジェクトを選択す
るのかを示す選択種別を入力して提供します。

構文::

    $<Name> = comdat SelectionKind

選択種別は、以下のいずれかの値を持つ必要があります。

``any``
    リンカは、任意のCOMDATキーを選択します。
``exactmatch``
    リンカは任意のCOMDATを選択しますが、そのセクションは同じデータを保
    持している必要があります。
``largest``
    リンカは、最も大きいCOMDATキーを持つセクションを選択します。
``noduplicates``
    リンカは、COMDATキーが重複していないこと要求します。
``samesize``
    リンカは、任意のCOMDATキーを選択しますが、そのセクションは同じサイ
    ズのデータを保持している必要があります。

Mach-Oプラットフォームは、選択種別として ``any`` のみをサポートする
COMDATとELFをサポートしません。

以下は、COMDATグループの例です。この例での関数はCOMDATキーのセクション
が一番大きい場合にのみ選択されます。

.. code-block:: llvm

   $foo = comdat largest @foo = global i32 2, comdat $foo

   define void @bar() comdat $foo { ret void }

COFFオブジェクトファイルの場合、選択種別が
``IMAGE_COMDAT_SELECT_LARGEST`` であり、 ``@foo`` シンボルの内容を含む
COMDATセクションを生成します。また、選択種別として
``IMAGE_COMDAT_SELECT_ASSOCIATIVE`` を指定することで最初のCOMDATセクショ
ンと関連させ、 ``@bar`` シンボルを内容として含むCOMDATセクションも生成
します。

大域オブジェクトの属性にはいくつか制限があります。対象がCOFFの場合、大
域オブジェクトもしくはそのエイリアスは、COMDATグループと同じ名前を持つ
必要があります。このオブジェクトの内容とサイズは、リンク時にどのCOMDAT
グループを選択するのかを決定するために使用されます。オブジェクトの名前
は、COMDATグループと一致している必要がありますが、もしオブジェクトのリ
ンケージが局所であった場合、シンボルテーブルで衝突が発生した際に名前が
変更される場合があります。このため、オブジェクトは大域である必要があり
ます。

COMDATとセクション属性の組み合わせにより、思わぬ結果になる場合がありま
す。例えば、
...
オブジェクトファイルの観点からは、これは2つのセクションを同じ名前で作成
することを要求しています。これは、オブジェクトファイルレベルでは、両方
の大域が異なるCOMDATグループとCOMDATに属するために必要です。

これは必要です。なぜなら、両方の大域は、異なるCOMDATグループとCOMDATに
所属するため、オブジェクトファイルレベルでは、セクションとして表現され
るためです。
...
'``getelementptr``'命令は、:ref:`集合体 <t_aggregate>`データ構造の部分
要素のアドレスを取得するために使用します。この命令は、アドレスの計算を
行うだけで、実際にメモリへのアクセスは行いません。

Arguments:
""""""""""

最初の引数は、常にポインタかポインタのベクタであり、この値を基準に計算
が行われます。その他の引数は、集合体オブジェクトのどの要素のインデック
スを作成するのかを決定します。インデックスの解釈は、インデックスを作成
する対象に依存します。最初のインデックスは、最初の引数として与えられた
値のポインタへのインデックスになります。二番目のインデックスは、そのポ
インタが指す型の値のインデックスとなります（最初のインデックスは、0には
ならないので、その値が直接的に指し示す必要はありません）。インデックス
化された最初の型は、ポインタである必要があります。その後の型は配列やベ
クタ、構造体になる場合があります。最初の型のインデックス化された型をポ
インタにすることはできません。その先のアドレスを取得するためには、ポイ
ンタを通じてメモリにアクセスする必要があるためです。

インデックスの引数の型は、インデックス化する対象の型に依存します。構造
体のメンバへのインデックスを作成する場合、``i32``整数定数のみが使用でき
ます（ベクタのインデックスの場合、すべて同じ``i32``整数定数である必要が
あります）。配列やポインタ、ベクタをインデックスする場合、いかなるビッ
ト幅の整数でも使用することができます。また、定数である必要もありません。
これらの整数は、適切な符号付きの値として扱われます。

例として、以下のCコードがどのようにLLVMにコンパイルされるのかを見てみましょう。
...
上の例では、最初のインデックスは、'``%struct.ST*``'を指します。二番目の
インデックスは、構造体の三番目の要素を指します。三番目のインデックスは、
構造体の二番目の要素を指します。二次元配列は、'``i32``'型へのなりま
す。'``getelementptr``'命令は、この要素へのポインタを返します。このた
め、'``i32*``'型の値を計算します。

構造体の内部の要素へのポインタを返しても問題ありません。このため、上記
の例は以下のように書き換えることもできます。
...
``inbounds``キーワードを付加した場合、ベースポインタが、オブジェクトの
`inbound`なアドレスではない場合や、あるオブジェクトを指すベースアドレス
にオフセットを連続的に加算して得られるアドレスのいずれかがオブジェクト
のinboundsなアドレスではない場合、``getelementptr``の結果として得られる
値は、:ref:`poison value <poisonvalues>`になる場合があります。オブジェ
クトのinboundsなアドレスとは、そのオブジェクト内のすべてのアドレスとオ
ブジェクトの最後から1バイトを含む領域のことを意味します。ベースアドレス
がポインタのベクタの場合、``inbounds``キーワードは、その各要素のそれぞ
れに適用されます。

``inbounds``キーワードが付加されない場合、オフセットは2の補数表現に変換
した後に、ベースアドレスに加算されます。オフセットがポインタとは異なる
ビット幅を持つ場合、符号拡張をするかポインタのビット幅に合わせられます。
``getelementptr``の結果として得られる値は、ベースポインタが指すオブジェ
クトの外部となる可能性があります。その値がたまたま他のオブジェクトを指
している場合でも、その値を使用することは勧められません。より多くの情報
が必要な場合は、:ref:`Pointer Aliasing Rules <pointeraliasing>`を参照し
てください。

``getelementptr``命令は、紛らわしいです。この命令の振る舞いについてより
知りたい場合は、:doc:`the getelementptr FAQ <GetElementPtr>`を参照して
ください。
